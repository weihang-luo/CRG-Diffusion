...
class Default_Conf(NoneDict):
    ...
    def get_dataloader(self, dset='train', dsName=None, classes=None,):
        candidates = self['data'][dset]
        ds_conf = candidates[dsName].copy()
        return load_data_inpa(**ds_conf,classes=classes,)
    ...
    def eval_imswrite(self, srs=None, fsrs=None, img_names=None, dset=None, name=None, ext='png', lrs=None, gts=None, gt_keep_masks=None, verify_same=True):
        ...
        if fsrs is not None:
            fsr_dir_path = expanduser(self['data'][dset][name]['paths']['srs']+img_names[0].split('_')[0])
            write_images(fsrs, img_names, fsr_dir_path)
