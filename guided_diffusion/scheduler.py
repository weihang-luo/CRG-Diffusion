...
def get_schedule_sample_final(t):
    return list(range(t)) + list(range(t, -1, -1)) + [-1]
...
def get_schedule_jump(t_T, n_sample, jump_length, jump_n_sample,
    ...
    while t >= 1:
        ...
        if (
            ...
            jumps[t] = jumps[t] - 1
            for _ in range(jump_length):                       ###### jumps[t]+2   jump_length
                t = t + 1
                ts.append(t)
            jumps2 = {}
            for j in range(0, t_T - jump2_length, jump2_length):
...
# 逐步降低回溯步长
def get_schedule_jump2(t_T, n_sample, jump_length, jump_n_sample,
                      jump2_length=1, jump2_n_sample=1,
                      jump3_length=1, jump3_n_sample=1,
                      start_resampling=100000000):
    ...
    while t >= 1:
        ...
        if (
            ...
            jumps[t] = jumps[t] - 1
            for _ in range(jumps[t]+1):                       ###### jumps[t]+2   jump_length
                t = t + 1
                ts.append(t)
