...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, conf=None, **kwargs):
        self.use_timesteps = set(use_timesteps)  # timestep_respacing  从1000步中均匀抽出250个步的list
        self.original_num_steps = len(kwargs["betas"])  # 原始的  1000个betas
        self.conf = conf

        base_diffusion = GaussianDiffusion(conf=conf,
                                           **kwargs)  # 初始化betas相关过程参数
        if conf.respace_interpolate:
            ...
            for i, alpha_cumprod in enumerate(base_diffusion.alphas_cumprod):  # 获得use_timesteps步长对于的betas
                if i in self.use_timesteps:
                    new_betas.append(1 - alpha_cumprod / last_alpha_cumprod)
                    last_alpha_cumprod = alpha_cumprod
                    self.timestep_map.append(i)
        kwargs["betas"] = np.array(new_betas)  # 更新参数 重新初始化新步数对于的Diffusion
